...
<body>
	<?php
		...
		$checkSubmit = true;
		function printTable($name,$gender,$address,$customer_type,$start_number,$end_number) {
			$sum_number = $end_number - $start_number;
			echo ('Ten Khach Hang : '.$name);
			echo ('Gioi Tinh : '.$gender);
			echo ('Dia Chi : '.$address);
			echo ('Loai Khach Hang : '.$customer_type);
			echo ('Tong So Dien : '.$sum_number);
			echo ('Tong So Tien Phai Tra : '.Cal($number, $customer_type));
		}
